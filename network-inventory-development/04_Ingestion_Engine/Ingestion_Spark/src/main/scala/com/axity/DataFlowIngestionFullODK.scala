...
object DataFlowIngestionFullODK {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val structTypeList = args(11)
    val SID_TID_EID = args(12).toString
    ...
    if(uriAvroDir != null && hiveTable != null && idTable!= null && hiveWriteFormat!=null && colsToHashID!=null && uriAvroCrifrasControlDirInput!=null){
      ...
      val structTypesSchemaList = structTypeList.split(",")
      val struct = StructType( structTypesSchemaList.map(x => StructField(x, StringType, true)))

      val timestampfile =  getDate()

      // load avro as DF
      val avroDeltaDF = spark.read.option("header", "false").option("delimiter","|").schema(struct).csv(uriAvroDir).withColumn("ctl_file_name",lit(uriAvroDir)).withColumn("ctl_file_date",lit(timestampfile.toLong)).cache()
      ...
      val yearAvro = timestampfile.substring(0,4)
      val monthAvro = timestampfile.substring(4,6)
      val dayAvro = timestampfile.substring(6,8)
      ...
      if(full.toString.trim ==full_str){
        ...
        val insertCount = fullDF.count()
        insertAvroCifrasControlODK(spark,uriAvroCrifrasControlDirInput,uriAvroCrifrasControlDirOutput,countRead,insertCount,SID,TID,EID,uriAvroDir,timestampfile)
      }
      else
