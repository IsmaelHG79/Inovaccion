-- DDL Script to Build Control Model for Repository Central Inventory (RCI)
-- Axity
-- April, 2020
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
-- Ejecutar en impala
--DROP TABLE IF EXISTS rci_db_metadata.cfg_rci_source;
create table rci_db_metadata.cfg_rci_source
(
...
-- Ejecuta en impala
--drop table if exists rci_db_metadata.cfg_rci_table;
create table rci_db_metadata.cfg_rci_table
(
...
-- Ejecutar en impala
--drop table if exists rci_db_metadata.cfg_rci_table_config;
create table rci_db_metadata.cfg_rci_table_config
(
...
-- Ejecuta en impala
--drop table if exists rci_db_metadata.cfg_rci_nifi;
create table rci_db_metadata.cfg_rci_nifi
(
	ctl_tid					int		    comment 'Identificador único de la tabla asociada a la configuración.',
	environment_config		string		comment 'Id del ambiente asociado a la configuración, DEV=Desarrollo, QA=Pruebas, PRO=Producción.',
...
-- Ejecuta en impala
--drop table if exists rci_db_metadata.cfg_rci_config_orchestration;
create table rci_db_metadata.cfg_rci_automation_config
(
	ctl_tid			       		int		    comment 'Identificador único de la tabla asociada a la configuración.',
	environment_config			string		comment 'Id del ambiente asociado a la configuración, DEV=Desarrollo, QA=Pruebas, PRO=Producción.',
...
-- Ejecuta en impala
--drop table if exists rci_db_metadata.cr_rci_validation;
create table rci_db_metadata.cr_rci_validation
(
	ctl_tid						int			comment 'Identificador único de la tabla asociada a la configuración.',
...
-- Ejecuta en impala
--drop table if exists rci_db_metadata.cr_rci_nifi_status_detail;
create table rci_db_metadata.cr_rci_nifi_process_detail
(
...
-- Ejecuta en impala
--drop table if exists rci_db_metadata.cr_rci_events;
create table rci_db_metadata.cr_rci_events
(
...
-- Ejecuta en impala
--drop table if exists rci_db_metadata.cr_rci_ingestion_file;
create table rci_db_metadata.cr_rci_ingestion_file
(
	...
	ctl_tid						int			comment 'ID de la tabla asociada a la ejecución',
	ctl_eid                     string      comment 'Es el ID con el que reconoceremos la carga dentro del proceso',
...
-- Ejecuta en impala
--drop table if exists rci_db_inventory.cr_rci_statistics;
create table rci_db_inventory.cr_rci_statistics
(
...
create table rci_db_metadata.cg_rci_events_catalog
(
