...
#!/bin/bash
#------------------------------------------------------------------------------------------------------------#
#	Nombre:		rci_run_flow_alcatel.sh                                          			 		 	 	 #
#	Versión:	1.0.0                                          			 		                             #
#	Objetivo: 	Shell encargado de ejecutar el flujo de ingestion de la fuente Alcatel						 #
#	Autor:		ejesus				                                                                         #
...
if [ $# -lt 1 ]
	then
		...
		echo "usage: sh $(basename $0)  environment"
...
. $path_log4jadc/log4j_attdlkrci.sh 2 'rci_run_flow_alcatel' "$environment" "$path_rci" "$group_exec" "$table_execute"
create_dir_logs
