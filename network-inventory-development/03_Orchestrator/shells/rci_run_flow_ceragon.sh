...
#!/bin/bash
#------------------------------------------------------------------------------------------------------------#
#	Nombre:		rci_run_flow_ceragon.sh                                          		 		 	 		 #
#	Versión:	1.0.0                                          			 		                             #
#	Objetivo: 	Shell encargado de ejecutar el flujo de ingestion de la fuente Ceragon						 #
#	Autor:		ejesus				                                                                         #
...
if [ $# -lt 1 ]
	then
		...
		echo "usage: sh $(basename $0)  enviroment"
		echo "enviroment          Ambiente en el que se desea ejecutar la fuente dev=Desarrollo, prd=Producción"
		echo '--------------------------------------------------------------'
	exit 1;
fi
...
. $path_log4jadc/log4j_attdlkrci.sh 2 'rci_run_flow_ceragon' "$environment" "$path_rci" "$group_exec" "$table_execute"
create_dir_logs
