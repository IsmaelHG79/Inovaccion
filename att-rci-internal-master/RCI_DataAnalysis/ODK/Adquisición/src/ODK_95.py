...
conf = SparkConf().setAppName('SND')      .setMaster('yarn').set("spark.yarn.queue","root.eda")
...
df = spark.sql("select * from rci_network_db.tx_odk where cve_type = 'SND' and substr(registry_state,1,10) = '2020-02-04' ")


# In[8]:


#Se hace una limpieza de los datos en la columna ma
...
df_1 = df_3.withColumn('map', trim(df_3.ma))
...
def clean_str(string):
    ...
    str_clean_3 = str_clean_2.replace(" ","_").replace("-","_").replace("activo_fijo","activo").replace("¿","").replace("?","") 
    return str_clean_3
...
df8.write.format("parquet").mode("overwrite").saveAsTable("rci_network_db.tx_stg_tabla_columnar_odk_95")
